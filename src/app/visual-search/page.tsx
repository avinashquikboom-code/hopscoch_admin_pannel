...
export default function VisualSearchPage() {
  const [visualSearches, setVisualSearches] = useState(initialVisualSearches);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [isProcessing, setIsProcessing] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');

  // Selected item for details drawer
  const [selectedItem, setSelectedItem] = useState<typeof initialVisualSearches[0] | null>(null);
  ...
  const handleUpload = async () => {
    ...
    setTimeout(() => {
      ...
      setVisualSearches([newItem, ...visualSearches]);
            ...
            <div className="space-y-4">
              {filteredSearches.map((item, idx) => (
                ...
                >
                  ...
                  <div className={`w-full md:w-36 h-28 rounded-lg bg-gradient-to-tr ${mockGradients[idx % mockGradients.length]} flex items-center justify-center shrink-0 relative overflow-hidden shadow-inner`}>
                    <ImageIcon className="h-6 w-6 text-white/50 animate-pulse" />
                  </div>
            ...
            {selectedItem && (
              <>
                ...
                <ScrollArea className="flex-1 p-6 space-y-6 h-full overflow-y-auto">
                  ...
                  {/* Left thumbnail mockup layout */}
                  <div className={`w-full h-40 rounded-xl bg-gradient-to-tr ${mockGradients[parseInt(selectedItem.id) % mockGradients.length]} flex flex-col justify-between p-5 shadow-inner relative overflow-hidden`}>
                    ...
                    <div className="p-4 rounded-xl border border-border/30 bg-muted/15 space-y-4">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex gap-2">
                          {selectedItem.metadata.colors.map((hex, idx) => (
                            <div key={idx} className="flex items-center gap-1.5 bg-background border border-border/40 p-1.5 rounded-lg select-all">
                              <div className="w-4 h-4 rounded-full shadow-sm" style={{ backgroundColor: hex }} />
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          ...
                          <div className="flex gap-1.5 flex-wrap">
                            {selectedItem.metadata.patterns.map((p, idx) => (
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="flex gap-1.5 flex-wrap">
                            {selectedItem.metadata.style.map((s, idx) => (
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedItem.similarProducts.map((sku, idx) => (
